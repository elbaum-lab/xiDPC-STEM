function panther_iDPC_function_PK4(varargin)
...
% crops 8 pixels form each side to avoid black strips in DiDPC1
...
% Convert string inputs to numeric values
    ...
    %bf_disk = str2double(bf_disk_str);
    angle_ind=0;
...
%fprintf(fid, 'BF disk is goint out to the : %s\n\n\n', bf_disk_str);
...
for caseno=casenovector
    for channelno=1:4
        ...
        if channelno==1
            ...
            %fprintf('This is the iDPC file I will write: %s\n', newFilename)
            newFilename1=fullfile(work_directory, strrep(workfile,'1_tilt000.mrc','_iDPC1.mrc'));
            %fprintf('This is the iDPC1 file I will write: %s\n', newFilename1)
            newFilename2=fullfile(work_directory, strrep(workfile,'1_tilt000.mrc','_iDPC2.mrc'));
            newFilename11=fullfile(work_directory, strrep(workfile,'1_tilt000.mrc','_DiDPC1_noFilt.mrc'));
            fprintf('This is the unfiltered DiDPC1 file I will write: %s\n', newFilename11)
            newFilename12=fullfile(work_directory, strrep(workfile,'1_tilt000.mrc','_DiDPC1_high.mrc'));
            fprintf('This is the filtered DiDPC1 file using a high number (300) I will write: %s\n', newFilename12)
            newFilename13=fullfile(work_directory, strrep(workfile,'1_tilt000.mrc','_DiDPC1_low.mrc'));
            ...
            %newFilename=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_iDPC.mrc');
            %newFilename1=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_iDPC1.mrc');
            %newFilename2=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_iDPC2.mrc');
            %newFilename11=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_DiDPC1.mrc');
            %newFilename3=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_deshift_SUM.mrc');
            %newFilenameX=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_DPCx.mrc');
            %newFilenameY=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_DPCy.mrc');
            %newFilename4=strrep(Chosen_Filename_ch,'1_tilt000.mrc','_plain_SUM.mrc');
        end
...
%        end
        ...
        comx_rotj=img_gradx*cos(angle_ind*pi/180)-img_grady*sin(angle_ind*pi/180);
        comy_rotj=+img_gradx*sin(angle_ind*pi/180)+img_grady*cos(angle_ind*pi/180);
        img_gradx=comx_rotj;
        img_grady=comy_rotj;
    ...
    fprintf(fid, 'I have written the unfiltered DiDPC file: %s\n', newFilename11)
    close(newmRCImage);
    ...
    if false
        ...
        fprintf(fid, 'I have written the filtered DiDPC using a high number (300) file: %s\n', newFilename12)
        close(newmRCImage);
        ...
        fprintf(fid, 'I have written the filtered DiDPC using a low number (50)  file: %s\n', newFilename13)
        close(newmRCImage);
